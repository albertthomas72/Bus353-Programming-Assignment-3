...
//Rare exceptions to indicate a bug in the _map code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_map = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_map]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.map.module"
